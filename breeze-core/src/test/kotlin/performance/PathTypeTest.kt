...
class PathTypeTest {
	...
	fun matchesByVsMatchesTest() {
		val n = 1000000
		val executor = Executors.newFixedThreadPool(3)
		val f1 = executor.submit(Callable { measureNanoTime { repeat(n){"/foo/bar/bar/bar".matchesBy("/foo/*/b?r/**", PathType.AntPath)} } })
		val f2 = executor.submit(Callable { measureNanoTime { repeat(n){"/foo/bar/bar/bar".matches("/foo/[^/?]*/b.r/.*".toRegex())} } })

		println("/foo/bar/bar/bar".matchesBy("/foo/*/b?r/**", PathType.AntPath))
