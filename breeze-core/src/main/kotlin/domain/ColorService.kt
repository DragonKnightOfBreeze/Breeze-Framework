...
@file:Suppress("ReplaceSubstringWithTake")
...
object ColorService {
	...
	fun parse(expression: String): Color {
		return doParse(expression) ?: throw IllegalStateException("Cannot parse '$expression' to color.")
	}

	fun parseOrNull(expression: String): Color? {
		return doParse(expression)
	}

	private fun doParse(expression: String): Color? {
		return when {
