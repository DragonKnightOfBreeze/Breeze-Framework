...
/**
 * 将当前的一组键值对转化成字符串，默认的转化规则如下：
 * * 没有有效键值对/值的数组 && 忽略空键值对 -> 直接返回空字符串
 * * 空键值对/值和空字符串 && 忽略空键值对 -> 忽略该键值对
 * * 存在转化方法 -> 使用转化方法转化键值对
 */
fun <K, V> Map<K, V>.typingAll(
	separator:CharSequence = ", ", prefix:CharSequence = "", postfix:CharSequence = "",
	ignoreEmpty:Boolean = true, ignoreEmptyValue:Boolean = true,ignoreEmptyEntry:Boolean = true,
	transform:((Map.Entry<K, V>) -> CharSequence)? = null
):String = buildString {
	var count = 0
	for(entry in this@typingAll) {
		val valueSnippet = entry.value?.toString()
		if(ignoreEmptyValue && valueSnippet.isNullOrEmpty()) continue
		val snippet = if(transform == null) entry.toString() else transform(entry)
		if(ignoreEmptyEntry && snippet.isEmpty()) continue
		if(count++ > 0) append(separator)
		append(snippet)
	}
	if(ignoreEmpty && length > 0) insert(0, prefix).append(postfix)
